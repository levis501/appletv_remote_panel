...
async def pair_protocol(atv_config, protocol_name):
    ...
    print("  A PIN should appear on your Fruit TV screen.")

    try:
        pairing = await pyatv.pair(atv_config, protocol, asyncio.get_event_loop())
        await pairing.begin()

        pin_str = input("  Enter the PIN shown on your Fruit TV: ").strip()
    ...
    for atv_config in chosen:
        ...
        if ATTEMPT_MRP:
            ...
            if mrp_creds:
                ...
                print("    - Fruit TV rejected the pairing request (try re-running setup)")
...
async def setup():
    ...
    while True:
        ...
        print("\n=== Fruit TV Remote — Device Manager ===\n")
